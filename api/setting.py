...
def parse_model_settings(yaml_file):
    ...
    model_settings = []
    for i in models:
        # Expand environment variables in compartment_id
        compartment_id = os.path.expandvars(i.get('compartment_id', ''))
...
for m in MODEL_SETTINGS:
    if m['type'] == "embedding":
        ...
        # Infer provider from model id prefix when not explicitly set

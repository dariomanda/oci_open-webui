...
class OCIGenAIModel(BaseChatModel):
    ...
    def _parse_request(self, chat_request: ChatRequest) -> oci_models.ChatDetails:
        ...
        if provider == "cohere":
        ...
        elif provider in ("meta", "openai", "google", "xai"):
            ...
            generic_chatRequest.messages = generic_messages
        ...
        if "finishReason" not in chunk:
            if model_id.startswith("cohere"):
                if "tooCalls" not in chunk:
                    ...
                    #         tool_calls=openai_tool_calls
                    #         )
            elif model_id.startswith("meta") or model_id.startswith("openai") or model_id.startswith("google") or model_id.startswith("xai"):
            ...
            if "toolCalls" in chunk:
                openai_tool_calls = Convertor.convert_tool_calls_llama_to_openai(chunk["toolCalls"])
